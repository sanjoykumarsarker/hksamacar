...
//   $new_num = str_replace($en_num, $bn_num, $en_price);
  ...
  $vpno=substr($row["VPNo"],2);
           ...
           <div class="page">
            <br>             
            <h4 style="font-size:20px; text-align: left; font-weight: bold;">ভিপিপি মূল্য: ('.$banglaNum->engToBn($en_price).'/-) '.$banglaNum->numToWord($en_price).' টাকা</h4> <br><br>
            <br> 
           ...
           <div style="font-family:SutonnyMJ;font-size:28px; text-align: left;  font-weight: bold; "> &nbsp&nbsp ৳'.$banglaNum->engToBn($en_price).'/- </div>
           <span style="font-size:20px; text-align: left;  font-weight: bold; "> &nbsp  &nbsp &nbsp   &nbsp  &nbsp &nbsp VPP:'.str_pad(intval($vpno),3,"0",STR_PAD_LEFT).'</span><br>
        ...
        ডাকঃ '.id_po_bn($row["transid"]).",থানাঃ ".ps_en_bn(id_ps($row["transid"])).', <br>
         জেলা: '.dist_en_bn(id_dist($row["transid"])).' <br>
           ...
           <div style="font-size:18px; font-weight: bold; text-align:center;">পার্সেল-'.number_en_bn($journal).'</div>
                <svg class="barcode"
        ...
        // <svg style="width:320px;height:70px;" id="s'.$row["transid"].'" ></svg>
