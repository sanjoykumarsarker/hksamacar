...
$range2="";
                ...
                if(isset($_POST["range"]) && !empty($_POST["range"])){
                    ...
                    $pieces = explode("-", $range);
                     $range1=  $pieces[0]; // piece1
                    $range2= $pieces[1]; // piece2
//$extra_sql="LIMIT ".intval($range2-$range1+1)." OFFSET ".intval($range1-1);

 $extra_sql="and reg between cast((select reg from reg where reg like '%$range1'and templereg='$tid' ) as unsigned) and cast((select reg from reg where reg like '%$range2' and templereg = '$tid') as unsigned)";

//reg between cast((select reg from reg12082022 where reg like '%012'and templereg='021001' ) as unsigned) and cast((select reg from reg12082022 where reg like '%057' and templereg='021001') as unsigned)
...
echo "Showing :".$range1."-".$range2."";
                ...
                if ($connr->connect_error) {
                    ...
                    $sqlr = "SELECT reg,bname,ename,spouse_id,spouse_name,templename,age,address,service,gender,mstatus,nprefer,education,phone,
	recom,nchoice1,nchoice2,nchoice3,finalname,finalname_b,comments,imagename
	FROM reg WHERE  templereg='$tid' and isinitiated!='Initiated' and result_viva='fit' and isinitiated!='Absent'  ".$extra_sql;
	...
	<th width= 200 >Image</th> <th>Devotee Information</th>		<th width= 300> Choice of Name</th>
		...
		<td><form target='_blank' action='devedit.php' method='POST'><button name='edit' class= 'tbutton' type='submit' value=" . $row["reg"] . ">Reg: " . $row["reg"] . "</button>
			...
			Name:<span style='color:red;'> " . $row["ename"] . "</span> <hr>
			 " . $row["bname"] . "   <hr>
          <span style='color:purple'>   Spouse: " . $row["spouse_id"] ." ". $row["spouse_name"]."  ".ename($row["spouse_id"] )." </span> <hr>
        ...
        <marquee style='color:purple'>   " . $row["spouse_id"] ." ". $row["spouse_name"]."  ".ename($row["spouse_id"] )." </marquee> <hr>
    ...
    <div id="row" style="padding: 5px; text-align: center;">
        ...
        <form method="post" action="namakarana_one.php" target="_blank" style="cursor: pointer; text-align: center;">
            <a class="btn btn-info" onclick="myFunction()"> Reload </a>

            <a class="btn btn-info" onclick="window.location = 'namanirnaya_dashboard.php';"> Go To Dasboard
            </a>
            <input type="hidden" name="edit" value="<?php echo $tid; ?>">
            <input class="btn btn-info" type="submit" value="Go To Namakarana">
            <a class="btn btn-danger" onclick="myFunction()"> Show All </a>
        </form>


        <form method="post" action="namanirnaya_details.php" target="_blank">
         
            <input type="hidden" name="edit" value="<?php echo $tid; ?>">
            <input class="btn btn-info" type="submit" value="Namanirnaya Details">
         </form>
