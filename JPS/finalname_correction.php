...
$resultr=  $connr->query("SELECT reg,bname,ename,spouse_name,spouse_id,templename,age,address,service,gender,mstatus,nprefer,education,phone,
  ...
  while($row = $resultr->fetch_assoc()) {
    ...
    <td>
      ...
      <p>
        ...
        Spouse:   <span style="color:orange"> '.$row["spouse_name"].ename($row["spouse_id"]).'-'.$row["spouse_id"].'</span><br>
        '.$row["gender"].' (<span style="color:red;"> '.$row["mstatus"].'</span>)&nbsp; &nbsp;
        Age:  '.$row["age"].' , Lila: <span style="color:red;"> '.$row["nprefer"].'</span>
        <br />
        '.$row["service"].', Education:  '.$row["education"].'<br>Temple:'.$row["templename"].'
