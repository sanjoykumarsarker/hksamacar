...
<body class="A4 <?= $mode ?>">
    ...
    <?php if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $conn = make_connection();
        $select = "SELECT bname, ename, finalname_b, finalname,
                         reg, iserial, idate
                FROM reg
                 WHERE idate='$date' and result_viva='fit' ";

        $order = "ORDER BY CONVERT(iserial, UNSIGNED INTEGER)";

        if (!empty($_POST['search'])) {
            $search = filter_var($_POST['search'], FILTER_SANITIZE_STRING);
            $query = " AND (ename LIKE '%$search%' OR phone LIKE '$search%' OR bname LIKE '%$search%' OR finalname LIKE '%$search%') ";
            $select .= $query;
        }
        ...
        if ($resultd->num_rows > 0) {
            while ($row = $resultd->fetch_assoc()) {
                ...
                <div class="container">
                    ...
                    <div class="marquee">শ্রীল জয়পতাকা স্বামী গুরুমহারাজ</div>
                    <div class="assignment" style="color:#666;">
