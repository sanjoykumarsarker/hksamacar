...
<head>
 ...
 $flag_gen=1;
  ...
  $nch1= strtoupper($_POST['finalname']);
 
  $finalname_b= strtoupper($_POST['finalname_b']);

  $idate= strtoupper($_POST['idate']);
  $iserial= strtoupper($_POST['iserial']);
...
if ($resultn->num_rows > 0) {
    echo "<table width=100% style='  box-shadow: none;' ><tr><th colspan=2 style='text-align:center;' > </th></tr> <tr><th>  </th> <th></th></tr>";
    // output data of each row
...
$sqlu= "UPDATE reg SET finalname='$nch1' ,finalname_b='$finalname_b' ,idate='$idate',iserial='$iserial',namakarana_time=now()  WHERE reg= '$regid'";

 
 $sqlu_date= "UPDATE date_set SET idate='$idate',iserial='$iserial'  WHERE reg= '$regid'";

 if($flag_gen==1){
 $resultu = $conn->query($sqlu);

 $resultu_date = $conn->query($sqlu_date);
...
alert(nch1+" Corrected Successfully!!!");
  </script>
