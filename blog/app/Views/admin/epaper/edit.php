...
<div class="card card-body border-0 shadow mb-4">
    <form method="post" action="<?= url_to('admin.epapers.update', $epaper->id) ?>" enctype="multipart/form-data">
        <?= csrf_field() ?>
        <div class="row">
            <div class="col-md-4">
                <?= make_field('issue', ['data' => $epaper->issue]) ?>
            </div>
            <div class="col-md-4">
                <?= make_field('issue_no', ['data' => $epaper->issue_no]) ?>
            </div>
            <div class="col-md-4">
                <?= make_field('created_at', ['data' => $epaper->created_at], 'date') ?>
            </div>
        </div>
        <div class="row">
            <div class="col-md-8">
                <?= make_field('body', ['data' => $epaper->body], 'textarea') ?>
            </div>
            <div class="col-md-4">
                <div class="input-group   mb-3">
                    ...
                    <select class="form-select" name="type" id="type">
                        <?php foreach (['pdf', 'image'] as $option) : ?>
                            <option value="<?= $option ?>" <?= old('type', $epaper->type) === $option ? 'selected' : '' ?>><?= strtoupper($option) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <?= make_field('active', ['data' => $epaper->active], 'switch') ?>
            </div>
