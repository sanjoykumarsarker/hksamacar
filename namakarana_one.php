...
<div> 
<img src="header.png" style=" padding: 0px; position:fixed; z-index:100;" height="100px" width="100%">
</div>
...
<div style="  background-color: cornsilk ; margin-top: 75px; " >

 	<div id="" style="overflow-y: scroll; height:430px;">
...
$sqlr = "SELECT reg,
	ename,spouse_name,spouse_id, nchoice1,nchoice2,nchoice3
...
if ($resultr->num_rows > 0) {
	...
	<tr>
	<th width= 100> Reg</th>		<th> Name</th>	<th> Spouse</th>	 
	<th>❶ </th>	<th>❷</th>			<th>❸</th>	
	</tr>";
	// output data of each row
    while($row = $resultr->fetch_assoc()) {
		...
		<td><form target='myframe1'action='namakarana_data_one.php' method='POST'><button class= 'tbutton' name='rid' type='submit' value=".$row["reg"]." >".$row["reg"]."</button> </form></td>
		<td>".$row["ename"]."</td>
    <td>".$row["spouse_name"].$row["spouse_id"]."</td>
		<td>".$row["nchoice1"]."</td>
		<td>".$row["nchoice2"]."</td>
...
?>
    ...
    <a class="btn btn-info" onclick="window.location = 'namakarana_dashboard.php';">Go to Dashboard</a>
