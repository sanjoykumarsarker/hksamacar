...
<?php
 ...
 $age = $sheet->getCell( 'D'.$row )->getValue();
  $occupation = $sheet->getCell( 'E'.$row )->getValue();
  $marital = $sheet->getCell( 'F'.$row )->getValue();
 $education = $sheet->getCell( 'G'.$row )->getValue();
 $address = $sheet->getCell( 'H'.$row )->getValue();
 $temple = $sheet->getCell( 'I'.$row )->getValue();
 $recommend = $sheet->getCell( 'J'.$row )->getValue();
  $lila = $sheet->getCell( 'K'.$row )->getValue();
...
$dbname = "sanpro_diksa";
      ...
      mysqli_set_charset($conn_all,"utf8");
       $sql_all = "SELECT ename,age,templereg FROM reg where templereg=".$temple_id."";
